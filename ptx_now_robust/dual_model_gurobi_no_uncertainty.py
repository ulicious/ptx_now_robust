...
class GurobiDualProblem:
    ...
    def attach_variables(self):
        ...
        self.y_weekly_production_balance_variable \
            = self.model.addVars(list(itertools.product(self.demanded_commodities, self.clusters)),
                                 name='weekly_production', lb=-gp.GRB.INFINITY, ub=gp.GRB.INFINITY)

        self.y_out_constraint_variable \
            = self.model.addVars(list(itertools.product(self.conversion_components, self.final_commodities, self.time, self.clusters)),
                                 name='x_out', lb=-gp.GRB.INFINITY, ub=gp.GRB.INFINITY)

        self.y_in_constraint_variable \
            = self.model.addVars(list(itertools.product(self.conversion_components, self.final_commodities, self.time, self.clusters)),
                                 name='x_in', lb=-gp.GRB.INFINITY, ub=gp.GRB.INFINITY)

        self.y_conv_cap_ub_constraint_variable \
            = self.model.addVars(list(itertools.product(self.conversion_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='cap_ub')
        ...
        self.y_conv_cap_ramp_up_constraint_variable \
            = self.model.addVars(list(itertools.product(self.conversion_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='cap_ramp_up')

        self.y_conv_cap_ramp_down_constraint_variable \
            = self.model.addVars(list(itertools.product(self.conversion_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='cap_ramp_down')

        self.y_generation_constraint_variable_active \
            = self.model.addVars(list(itertools.product(self.generator_components, self.generated_commodities, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='generation')
    ...
    def attach_relaxed_weekly_demand(self):
        ...
        for combi in combinations:
            ...
            if com in self.demanded_commodities:
                ...
                for t in self.time:
                    self.model.addConstr(
                        - self.y_balance_constraint_variable[com, t, cl] - self.y_demand_constraint_variable[com, cl] <= 0,
