...
class GurobiDualProblem:
    ...
    def attach_variables(self):
        ...
        self.y_free_available_constraint_variable \
            = self.model.addVars(list(itertools.product(self.available_commodities, self.time, self.clusters)), lb=0, name='free')

        self.y_purchase_constraint_variable \
            = self.model.addVars(list(itertools.product(self.purchasable_commodities, self.time, self.clusters)), lb=0, name='purchase')
        ...
        for combi in combinations:
            ...
            if com in self.storage_components:
                ...
                if t < max(self.time):
                    ...
                    lhs.append(- self.y_soc_constraint_variable[com, t, cl] * self.charging_efficiency_dict[com])

                self.model.addConstr(sum(lhs) <= self.variable_om_dict[com] * self.weightings[cl], name='x_charge_rule_' + name_adding)

                lhs = [self.y_balance_constraint_variable[com, t, cl] + self.y_soc_discharge_limit_constraint_variable[com, t, cl]]
                if t < max(self.time):
                    lhs.append(self.y_soc_constraint_variable[com, t + 1, cl] / self.discharging_efficiency_dict[com])
                else:
                    lhs.append(self.y_soc_constraint_variable[com, t, cl] / self.discharging_efficiency_dict[com])

                self.model.addConstr(sum(lhs) <= 0, name='x_discharge_rule_' + name_adding)

                lhs = []
                if t == max(self.time):
                    lhs.append(- self.y_soc_constraint_variable[com, 0, cl]
                               + self.y_soc_constraint_variable[com, t, cl]
    ...
    def optimize(self):
        ...
        def save_results():
            ...
            for p in self.profiles:
                if self.weighting_profiles_binary[p].X == 1:
                    print(p)
                    self.chosen_profiles = {'Wind': self.data['Wind_' + str(p)],
                                            'Solar': self.data['Solar_' + str(p)]}
        ...
        # self.model.write('P:/Group_TE/GM_Uwe/PtL Robust/gurobi.lp')
