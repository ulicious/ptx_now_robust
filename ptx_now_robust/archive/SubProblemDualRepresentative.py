...
class MixedIntDualRepresentative:
    ...
    def attach_commodity_variables_to_optimization_problem(self, dual_model):
        ...
        dual_model.y_generation_constraint_variable_active = Var(dual_model.GENERATORS,
                                                                 ...
                                                                 dual_model.PROFILES, bounds=(None, 0))
    ...
    def attach_nominal_generation(self, dual_model):
        ...
        for c in self.data.columns:
            ...
            cluster_profile[n] = cluster_profile[n] + 1 # todo: what is this code doing?
    ...
    def attach_uncertainty_set(self, dual_model):
        ...
        # todo: delete unnecessary code and adjust rest with global booleans and global values chosen at initiation
        ...
        if True:
            ...
            def activate_active_generation(dm, g, n, p):
                generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                return 0 <= sum(dm.y_generation_constraint_variable_active[g, generated_commodity, t, n, p]
                                for t in dm.TIME) \
                       + dm.weighting_profiles_binary[n, p] * sum(dm.generation_profile[g, n, p, t]
                                                                  * self.optimal_capacities[g]
                                                                  for t in dm.TIME)
            dual_model.activate_active_generation_con = Constraint(dual_model.GENERATORS, dual_model.CLUSTER,
                                                                   dual_model.PROFILES, rule=activate_active_generation)

            def activate_inactive_generation(dm, g, n, p):
                generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                return 0 <= sum(dm.y_generation_constraint_variable_inactive[g, generated_commodity, t, n, p]
                                for t in dm.TIME) \
                       + (1 - dm.weighting_profiles_binary[n, p]) * sum(dm.generation_profile[g, n, p, t]
                                                                        * self.optimal_capacities[g]
                                                                        for t in dm.TIME)
            dual_model.activate_inactive_generation_con = Constraint(dual_model.GENERATORS, dual_model.CLUSTER,
                                                                     dual_model.PROFILES,
    ...
    def attach_constraints(self, model):
        ...
        def x_generation_rule(dm, g, s, t, n, p):
            generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
            if s == generated_commodity:
                return dm.y_balance_constraint_variable[s, t, n] \
                       + dm.y_generation_constraint_variable_active[g, s, t, n, p] \
                       + dm.y_generation_constraint_variable_inactive[g, s, t, n, p] <= dm.variable_om[g]
            else:
                return Constraint.Skip
        self.dual_model.x_generation_con = Constraint(self.dual_model.GENERATORS, self.dual_model.ME_COMMODITIES,
                                                      self.dual_model.TIME, self.dual_model.CLUSTER, self.dual_model.PROFILES,
        ...
        # todo: check objective functions and delete deprecated
            ...
            if True:
                return sum(dm.y_demand_constraint_variable[s] * self.demand_dict[s, 0] for s in dm.DEMANDED_COMMODITIES) \
                       + sum((dm.y_conv_cap_ub_constraint_variable[c, t, n] * self.maximal_power_dict[c]
                             ...
                             * self.optimal_capacities[c]
                             for t in dm.TIME for c in dm.CONVERSION_COMPONENTS for n in dm.CLUSTER) \
                       + sum(dm.y_generation_constraint_variable_active[g, self.pm_object.get_component(g).get_generated_commodity(), t, n, p]
                             * self.optimal_capacities[g]
                             * dm.generation_profile[g, n, p, t] for p in dm.PROFILES
                             for n in dm.CLUSTER for t in dm.TIME for g in dm.GENERATORS) \
                       + sum((dm.y_soc_ub_constraint_variable[s, t, n] * self.maximal_soc_dict[s]
                              - dm.y_soc_lb_constraint_variable[s, t, n] * self.minimal_soc_dict[s]
                              + dm.y_soc_charge_limit_constraint_variable[s, t, n] * self.ratio_capacity_power_dict[s]
    ...
    def __init__(self, pm_object, solver, optimal_capacities, data, number_clusters, weightings, number_profiles, **kwargs):
        ...
        # todo: what does "not used anymore" mean?
        ...
        # todo: implement cases with booleans and values
