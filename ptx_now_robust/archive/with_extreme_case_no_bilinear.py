...
class ExtremeCaseNoBilinear:
    ...
    def attach_commodity_variables_to_optimization_problem(self, dual_model):
        ...
        dual_model.y_generation_constraint_variable_active_certain = Var(dual_model.GENERATORS,
                                                                 dual_model.GENERATED_COMMODITIES,
                                                                 dual_model.TIME, dual_model.CLUSTER,
                                                                 bounds=(None, 0))

        dual_model.y_generation_constraint_variable_active_uncertain = Var(dual_model.GENERATORS,
                                                                           dual_model.GENERATED_COMMODITIES,
                                                                           dual_model.TIME, dual_model.PROFILES,
                                                                           bounds=(None, 0))

        dual_model.y_generation_constraint_variable_inactive_uncertain = Var(dual_model.GENERATORS,
                                                                             dual_model.GENERATED_COMMODITIES,
                                                                             dual_model.TIME, dual_model.PROFILES,
                                                                             bounds=(None, 0))
        ...
        dual_model.profile_binary = Var(dual_model.PROFILES, within=Binary)
    ...
    def attach_nominal_generation(self, dual_model):
        ...
        self.generation_profiles_dict = {}
        for g in [*self.data.keys()]:
            for c in self.data[g].columns:
                n = int(self.data[g].loc['cluster', c])
                if n != self.number_clusters:
                    for ind in self.data[g].index:
                        if ind != 'cluster':
                            if ind == max(dual_model.TIME):
                                break
                            self.generation_profiles_dict[(g, n, int(ind))] = self.data[g].loc[ind, c]

        dual_model.generation_profile_certain = Param(['Wind', 'Solar'], dual_model.CLUSTER, dual_model.TIME,
                                                      initialize=self.generation_profiles_dict)

        for g in [*self.data.keys()]:
            cluster_profile = {self.number_clusters: 0}
            for c in self.data[g].columns:
                n = int(self.data[g].loc['cluster', c])
                if n == self.number_clusters:
                    for ind in self.data[g].index:
                        if ind != 'cluster':
                            if ind == max(dual_model.TIME):
                                break
                            self.generation_profiles_dict[(g, cluster_profile[n], int(ind))] = self.data[g].loc[ind, c]
                    cluster_profile[n] = cluster_profile[n] + 1

        dual_model.generation_profile_uncertain = Param(['Wind', 'Solar'], dual_model.PROFILES, dual_model.TIME,
                                                        initialize=self.generation_profiles_dict)
    ...
    def attach_uncertainty_set(self, dual_model):
        ...
        if True:
            ...
            def balance_profile_binaries(dm):
                return sum(dm.profile_binary[p] for p in dm.PROFILES) == 1
            dual_model.balance_profile_binaries_con = Constraint(rule=balance_profile_binaries)

            def certain_generation_limitation(dm, g, n, t):
                generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                if n != max(dm.CLUSTER):
                    return Constraint.Skip
                else:
                    return 0 == dm.y_generation_constraint_variable_active_certain[g, generated_commodity, t, n]
            dual_model.certain_generation_limitation_con = Constraint(dual_model.GENERATORS, dual_model.CLUSTER,
                                                                   dual_model.TIME, rule=certain_generation_limitation)

            if False:

                def activate_active_generation(dm, g, p, t):
                    generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                    return 0 <= dm.y_generation_constraint_variable_active_uncertain[g, generated_commodity, t, p] \
                           + (dm.generation_profile_uncertain[g, p, t] * self.optimal_capacities[g] * dm.profile_binary[p])
                dual_model.activate_active_generation_con = Constraint(dual_model.GENERATORS, dual_model.PROFILES,
                                                                       dual_model.TIME, rule=activate_active_generation)

                def activate_inactive_generation(dm, g, p, t):
                    generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                    return 0 <= dm.y_generation_constraint_variable_inactive_uncertain[g, generated_commodity, t, p] \
                           + (dm.generation_profile_uncertain[g, p, t] * self.optimal_capacities[g] * (1 - dm.profile_binary[p]))
                dual_model.activate_inactive_generation_con = Constraint(dual_model.GENERATORS, dual_model.PROFILES,
                                                                         dual_model.TIME, rule=activate_inactive_generation)

            else:
                def activate_active_generation(dm, g, p, t):
                    generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                    return 0 <= dm.y_generation_constraint_variable_active_uncertain[g, generated_commodity, t, p] \
                           + (10000000000 * dm.profile_binary[p])
                ...
                def activate_inactive_generation(dm, g, p, t):
                    generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                    return 0 <= dm.y_generation_constraint_variable_inactive_uncertain[g, generated_commodity, t, p] \
                           + (10000000000 * (1 - dm.profile_binary[p]))

                dual_model.activate_inactive_generation_con = Constraint(dual_model.GENERATORS, dual_model.PROFILES,
                                                                         dual_model.TIME,
                                                                         rule=activate_inactive_generation)
            ...
            if False:
                def min_renewable_generation_rule(dm, g):
                    return sum(dm.generation_profile[g, n, p, t] * dm.weighting_profiles_binary[n, p] * dm.weightings[n]
                               for t in dm.TIME for n in dm.CLUSTER for p in dm.PROFILES) >= 0. * 8760
    ...
    def attach_constraints(self, model):
        ...
        def x_generation_rule(dm, g, s, t, n):
            ...
            if s == generated_commodity:
                if n != max(dm.CLUSTER):
                    return dm.y_balance_constraint_variable[s, t, n] \
                           + dm.y_generation_constraint_variable_active_certain[g, s, t, n] <= dm.variable_om[g]
                else:
                    return dm.y_balance_constraint_variable[s, t, n] \
                           + dm.y_generation_constraint_variable_active_certain[g, s, t, n] \
                           + sum(dm.y_generation_constraint_variable_active_uncertain[g, s, t, p]
                                 + dm.y_generation_constraint_variable_inactive_uncertain[g, s, t, p]
                                 for p in dm.PROFILES) <= dm.variable_om[g] * dm.weightings[n]
            else:
                return Constraint.Skip
        ...
        def objective_function(dm):
            return sum(dm.y_demand_constraint_variable[s] * self.demand_dict[s, 0] for s in dm.DEMANDED_COMMODITIES) \
                   ...
                   + sum(dm.y_generation_constraint_variable_active_certain[g, self.pm_object.get_component(g).get_generated_commodity(), t, n]
                         * self.optimal_capacities[g] * dm.generation_profile_certain[g, n, t]
                         for n in dm.CLUSTER if n != max(dm.CLUSTER)
                         for t in dm.TIME for g in dm.GENERATORS) \
                   + sum(dm.y_generation_constraint_variable_active_uncertain[g, self.pm_object.get_component(g).get_generated_commodity(), t, p]
                         * self.optimal_capacities[g] * dm.generation_profile_uncertain[g, p, t]
                         for p in dm.PROFILES for t in dm.TIME for g in dm.GENERATORS) \
