...
class MixedIntDualRepresentative:
    ...
    def attach_commodity_variables_to_optimization_problem(self, dual_model):
        ...
        dual_model.y_binary_balance_constraint_variable_positive = Var(dual_model.GENERATORS,
                                                                   dual_model.GENERATED_COMMODITIES, dual_model.CLUSTER,
                                                                   dual_model.PROFILES,
                                                                   bounds=(0, None))

        dual_model.y_binary_balance_constraint_variable_negative = Var(dual_model.GENERATORS,
                                                                   dual_model.GENERATED_COMMODITIES, dual_model.TIME,
                                                                       dual_model.CLUSTER,
                                                                   dual_model.PROFILES, bounds=(None, 0))
    ...
    def attach_uncertainty_set(self, dual_model):
        ...
        if True:
            ...
            def active_profile_negative_rule(dm, g, t, n, p):
                generated_commodity = self.pm_object.get_component(g).get_generated_commodity()
                return 0 <= dm.y_generation_constraint_variable[g, generated_commodity, t, n, p]\
                       + dm.weighting_profiles_binary[n, p] * 1000000
            dual_model.active_profile_negative_con = Constraint(dual_model.GENERATORS, dual_model.TIME,
                                                                dual_model.CLUSTER, dual_model.PROFILES,
                                                                rule=active_profile_negative_rule)
    ...
    def attach_constraints(self, model):
        ...
        def x_demand_rule(dm, s, t, n):
            if s in dm.DEMANDED_COMMODITIES:
        ...
        def x_generation_rule(dm, g, s, t, n, p):
            ...
            if s == generated_commodity:
                return dm.y_balance_constraint_variable[s, t, n] \
                       + dm.y_generation_constraint_variable[g, s, t, n, p] <= dm.variable_om[g]
            else:
                return Constraint.Skip
            ...
            if True:
                return sum(dm.y_demand_constraint_variable[s] * self.demand_dict[s, 0] for s in dm.DEMANDED_COMMODITIES) \
                       + sum((dm.y_conv_cap_ub_constraint_variable[c, t, n] * self.maximal_power_dict[c]
                             ...
                             * self.optimal_capacities[c]
                             for t in dm.TIME for c in dm.CONVERSION_COMPONENTS for n in dm.CLUSTER) \
                       + sum(dm.y_generation_constraint_variable[g, self.pm_object.get_component(g).get_generated_commodity(), t, n, p]
                             * self.optimal_capacities[g]
            ...
            if str(v) == 'weighting_profiles_binary':
                ...
                for i in v.index_set():
                    if round(variable_dict[i]) == 1:
                        ...
                        chosen_profiles['Wind'][cluster] = self.data.loc[index_new, self.data.columns[cluster*11 + profile]].array

            if str(v) == 'y_generation_constraint_variable':
                print('')

            if str(v) == 'y_binary_balance_constraint_variable_positive':
                print('')

            if str(v) == 'y_out_constraint_variable':
                print('')

            if str(v) == 'y_binary_balance_constraint_variable_negative':
                print('')
