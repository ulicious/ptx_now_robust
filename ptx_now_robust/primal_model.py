...
class SuperProblemRepresentative:
    ...
    def attach_generation_time_series_to_optimization_problem(self):
        ...
        for i in [*self.nominal.keys()]:  # iteration
            for g in [*self.nominal[i].keys()]:  # generator
                for cl in [*self.nominal[i][g].keys()]:  # cluster
                    for t in range(len(self.nominal[i][g][cl])):  # time

                        generation_profiles_dict[(g, t, cl, i)] = self.nominal[i][g][cl][t]
    ...
    def attach_constraints(self):
        ...
        def _mass_energy_balance_rule(m, me_out, t, cl, i):
            ...
            if commodity_object.is_available():
                equation_lhs.append(m.mass_energy_available[me_out, t, cl, i])
            if commodity_object.is_emittable():
                equation_lhs.append(-m.mass_energy_emitted[me_out, t, cl, i])
            if commodity_object.is_purchasable():
                equation_lhs.append(m.mass_energy_purchase_commodity[me_out, t, cl, i])
            if commodity_object.is_saleable():
                equation_lhs.append(-m.mass_energy_sell_commodity[me_out, t, cl, i])
            if commodity_object.is_demanded():
                equation_lhs.append(-m.mass_energy_demand[me_out, t, cl, i])
            if me_out in m.STORAGES:
                equation_lhs.append(
                    m.mass_energy_storage_out_commodities[me_out, t, cl, i] - m.mass_energy_storage_in_commodities[
                        me_out, t, cl, i])
            if me_out in m.GENERATED_COMMODITIES:
                equation_lhs.append(m.mass_energy_total_generation[me_out, t, cl, i])

            for c in m.CONVERSION_COMPONENTS:
                if (c, me_out) in self.output_tuples:
                    equation_lhs.append(m.mass_energy_component_out_commodities[c, me_out, t, cl, i])

                if (c, me_out) in self.input_tuples:
                    equation_rhs.append(m.mass_energy_component_in_commodities[c, me_out, t, cl, i])
