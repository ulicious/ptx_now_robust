...
class GurobiDualProblem:
    ...
    def attach_variables(self):
        ...
        # lb=gp.GRB.INFINITY, ub=0

        self.y_free_available_constraint_variable \
            = self.model.addVars(list(itertools.product(self.available_commodities, self.time, self.clusters)), lb=gp.GRB.INFINITY, ub=0, name='free')

        self.y_purchase_constraint_variable \
            = self.model.addVars(list(itertools.product(self.purchasable_commodities, self.time, self.clusters)), lb=gp.GRB.INFINITY, ub=0, name='purchase')

        self.y_emit_constraint_variable \
            = self.model.addVars(list(itertools.product(self.emittable_commodities, self.time, self.clusters)), lb=0, name='emit')

        self.y_sell_constraint_variable \
            = self.model.addVars(list(itertools.product(self.saleable_commodities, self.time, self.clusters)), lb=0, name='sell')

        self.y_balance_constraint_variable \
            = self.model.addVars(list(itertools.product(self.final_commodities, self.time, self.clusters)), name='balance', lb=-gp.GRB.INFINITY, ub=gp.GRB.INFINITY)
        ...
        self.y_soc_constraint_variable \
            = self.model.addVars(list(itertools.product(self.storage_components, self.time, self.clusters)),
                                 name='soc', lb=-gp.GRB.INFINITY, ub=gp.GRB.INFINITY)
        ...
        self.y_soc_ub_constraint_variable \
            = self.model.addVars(list(itertools.product(self.storage_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='soc_ub')

        self.y_soc_lb_constraint_variable \
            = self.model.addVars(list(itertools.product(self.storage_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='soc_lb')

        # lb=0, ub=gp.GRB.INFINITY

        self.y_soc_charge_limit_constraint_variable \
            = self.model.addVars(list(itertools.product(self.storage_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='soc_charge_limit')

        self.y_soc_discharge_limit_constraint_variable \
            = self.model.addVars(list(itertools.product(self.storage_components, self.time, self.clusters)),
                                 lb=-gp.GRB.INFINITY, ub=0, name='soc_discharge_limit')
    ...
    def attach_constraints(self):
        ...
        for combi in combinations:
            ...
            if com in self.available_commodities:
                self.model.addConstr(self.y_balance_constraint_variable[com, t, cl] + self.y_free_available_constraint_variable[com, t, cl] <= 0,
                                     name='x_free_rule_' + name_adding)

            if com in self.emittable_commodities:
                self.model.addConstr(- self.y_balance_constraint_variable[com, t, cl] + self.y_emit_constraint_variable[com, t, cl] <= 0,
                                     name='x_emit_rule_' + name_adding)

            if com in self.purchasable_commodities:
                self.model.addConstr(self.y_balance_constraint_variable[com, t, cl] + self.y_purchase_constraint_variable[com, t, cl]
                                     <= self.purchase_price_dict[com, cl, t] * self.weightings_dict[cl],
                                     name='x_buy_rule_' + name_adding)

            if com in self.saleable_commodities:
                self.model.addConstr(- self.y_balance_constraint_variable[com, t, cl] + self.y_sell_constraint_variable[com, t, cl]
            ...
            if com in self.storage_components:
                lhs = [- self.y_balance_constraint_variable[com, t, cl] + self.y_soc_charge_limit_constraint_variable[com, t, cl]]
                if t < max(self.time):
                ...
                self.model.addConstr(sum(lhs) <= self.variable_om_dict[com] * self.weightings[cl], name='x_charge_rule_' + name_adding)

                lhs = [self.y_balance_constraint_variable[com, t, cl]
                       + self.y_soc_discharge_limit_constraint_variable[com, t, cl] / self.discharging_efficiency_dict[com]]
                if t < max(self.time):
                ...
                if t == max(self.time):
                    lhs.append(self.y_soc_constraint_variable[com, t, cl]
                               + self.y_soc_ub_constraint_variable[com, t, cl]
                               - self.y_soc_lb_constraint_variable[com, t, cl]
        ...
        if self.demand_type == 'relaxed_weekly':
            ...
            self.model.setObjective(
                ...
                + sum(self.y_generation_constraint_variable_active[
                          g, self.pm_object.get_component(g).get_generated_commodity(), t, n]
                      * self.optimal_capacities[g] * self.generation_profiles_certain_dict[g, t, n]
                      for n in self.clusters for t in self.time for g in self.generator_components)
                + sum((self.y_soc_ub_constraint_variable[s, t, n] * self.maximal_soc_dict[s]
                       - self.y_soc_lb_constraint_variable[s, t, n] * self.minimal_soc_dict[s]
                       + self.y_soc_charge_limit_constraint_variable[s, t, n] / self.ratio_capacity_power_dict[s]
                       + self.y_soc_discharge_limit_constraint_variable[s, t, n] / self.ratio_capacity_power_dict[s]) *
    ...
    def __init__(self, pm_object, solver, optimal_capacities, nominal, data, number_clusters, weightings,
        ...
        self.clusters = range(0, number_clusters)
