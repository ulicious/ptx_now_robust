...
def _transfer_results_to_parameter_object(pm_object, model_type):
    ...
    for key in all_variables_dict['nominal_cap']:
        ...
        if component.get_component_type() == 'conversion':
            ...
            total_variable_costs = \
                sum(all_variables_dict['mass_energy_component_out_commodities'][(c, main_output, t)]
                    * variable_om[c] for t in range(pm_object.get_covered_period()))

            if component.get_shut_down_ability():
                total_start_up_costs = sum(all_variables_dict['restart_costs'][(c, t)]
            ...
            total_variable_co2_emissions = \
                sum(all_variables_dict['mass_energy_component_out_commodities'][(c, main_output, t)]
                    * variable_co2_emissions for t in range(pm_object.get_covered_period()))
            ...
            total_variable_costs = \
                sum(all_variables_dict['mass_energy_storage_in_commodities'][(c, t)]
                    * variable_om[c] for t in range(pm_object.get_covered_period()))
            ...
            if not component.get_uses_ppa():
                total_variable_costs = \
                    sum(all_variables_dict['mass_energy_generation'][(c, generated_commodity, t)]
                        * variable_om[c] for t in range(pm_object.get_covered_period()))
                ...
                total_variable_costs = \
                    sum(generator_profile.loc[generator_profile.index[t]]
                        * all_variables_dict['nominal_cap'][c] * component.get_ppa_price()
                        for t in range(pm_object.get_covered_period()))
        ...
        for k in [*variable_dict.keys()]:
            ...
            if variable == 'mass_energy_purchase_commodity':
                ...
                commodity_object.set_purchase_costs(commodity_object.get_purchase_costs()
                                                    + variable_dict[k] * purchase_price_dict[k[0], 0, k[1]])
    ...
    if len(pm_object.get_final_generator_components_names()) > 0:
        ...
        for generator in pm_object.get_final_generator_components_names():
            ...
            t_range = range(pm_object.get_covered_period())
            ...
            if capacity != 0:
                potential_generation = sum(
                    generator_profile.loc[generator_profile.index[t]]
                ...
                potential_generation = sum(generator_profile.loc[generator_profile.index[t]]
                                           for t in range(pm_object.get_covered_period()))
    ...
    if pm_object.get_uses_representative_periods():
        columns = [str(t) for t in range(pm_object.get_covered_period())]
    else:
    ...
    ordered_list = ['Freely Available', 'Purchase', 'Emitting', 'Selling', 'Demand', 'Charging',
